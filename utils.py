...
def get_cates_map(cate_file):
    ...
    cate_set = set()
    list_of_cates=[[241, 242, 243, 244]]
    with open(cate_file) as f_cate:
        for l in f_cate.readlines():
            if len(l) == 0: break
            l = l.strip('\n')
            items = [int(i) for i in l.split(' ')]
            iid, cate_ids = items[0]+1, items[1:]  ##make iid starts from 1
            list_of_cates.append(cate_ids)
            for cid in cate_ids:
                cate_set.add(cid)
            iidcate_map[iid] = cate_ids
    return iidcate_map, len(cate_set), list_of_cates

def compute_distance_matrix(list_of_lists, distance_file):
    if not os.path.exists(distance_file):
        
        n = len(list_of_lists)
        distance_matrix = [[0] * n for _ in range(n)]
        # First, calculate the raw distances.
        for i in range(n):
            for j in range(n):
                distance = Levenshtein.distance(str(list_of_lists[i]), str(list_of_lists[j])) 
                distance_matrix[i][j] = 1/(1+distance)

        # Then, L2 normalize each row of the matrix.
        for i in range(n):
            l2_norm = math.sqrt(sum([x**2 for x in distance_matrix[i]]))
            if l2_norm > 0:
                distance_matrix[i] = [x / l2_norm for x in distance_matrix[i]]
        numpy_matrix = np.array(distance_matrix)
        np.save(distance_file, numpy_matrix)
    else:
        numpy_matrix = np.load(distance_file)
...
def cc_at_k(actual, predicted, cate_map, topk, cate_num):
    sum_cc = 0.0
    sum_ild = 0.0   ##mean distance, we not use genre vectors but directly the embeddings
    num_users = len(predicted)
    true_users = 0
    cc_dict = {}
    item_num = len(cate_map)
    for i in range(num_users):
        cc_set = set()
        act_set = set(actual[i])
        pred_set = set(predicted[i][:topk])
        if len(act_set) != 0:
            #sum_recall += len(act_set & pred_set) / float(len(act_set))
            for iid in pred_set:
                if iid == 0 or iid > item_num:   ##pred_set为什么会有0?
                    continue
                cc_ids = cate_map[iid]
                for cid in cc_ids:
                    cc_set.add(cid)
            sum_cc += len(cc_set)/cate_num
            true_users += 1
    return sum_cc / true_users#, sum_ild / true_users
